	...
	public override void CompleteMission (MissionPlan plan)
	{
		...
		if (plan.m_result == MissionPlan.Result.Success) {
			...
			if (m_currentStatus == null || plan.m_targetActor.m_actor.m_status == m_currentStatus) {
				...
				player.notifications.AddNotification (GameController.instance.GetTurnNumber(), title, message, EventLocation.Contacts, false, plan.m_missionID);
				plan.m_targetActor.m_actor.notifications.AddNotification (GameController.instance.GetTurnNumber(), title, message, EventLocation.Contacts, false, plan.m_missionID);
			}
